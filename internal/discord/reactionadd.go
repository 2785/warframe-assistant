...
func (h *EventHandler) HandleMessageReactionAdd(
	s *discordgo.Session,
	...
	case verificationDialog:
		role, err := h.MetadataService.GetRoleRequirementForGuild(
			string(verificationDialog),
			r.GuildID,
		)
		if err != nil {
			h.sendReplyWithLogging(
				s,
				r.GuildID,
				r.ChannelID,
				r.MessageID,
				"Error retrieving role requirements for action, please try again later or contact bot maintainer for help!",
			)
			return
		}
		...
		if h.mustHaveRoleWithID(
			...
			role,
			r.GuildID,
			messageReplier(s, r.GuildID, r.ChannelID, r.MessageID),
			s,
		) {
...
func (h *EventHandler) handleVerificationDialogReaction(
	s *discordgo.Session,
	r *discordgo.MessageReactionAdd,
	d *dialogInfo,
) {
	logger := h.Logger.With(
		WithGuildID(r.GuildID),
		WithUserID(r.UserID),
		WithSubmissionID(d.SID),
	...
	case emojiCheckMark:
		...
		replyWithErrorLogging(
			replier,
			fmt.Sprintf(
				"Successfully verified submission '%s', hit %s to see the next one",
				d.SID,
				emojiThumbsUp,
			),
			logger,
		...
		if err != nil {
			...
			replyWithErrorLogging(
				replier,
				"Could not add reaction for going to the next entry."+internalError,
				logger,
			)
			return
		}
	case emojiThumbsUp:
		...
		if err != nil {
			logger.Error(
				"could not drop cache entry",
				zap.Error(err),
				zap.String("cache-key", d.CacheKey),
			)
		}
	...
	case emojiCrossMark:
		...
		replyWithErrorLogging(
			messageReplier(s, d.GID, d.CID, d.MID),
			fmt.Sprintf(
				"Okay, please update the score or hit %s to remove this submission",
				emojiThumbsDown,
			),
			logger,
		)
	...
	case emojiThumbsDown:
		...
		if err != nil {
			...
			replyWithErrorLogging(
				messageReplier(s, d.GID, d.CID, d.MID),
				"Could not removing the entry."+internalError,
				logger,
			)
			return
		}
